...
- **用神 / favourable-element selection** (`bazi.ts:303-316`): strong → favour 食傷/財/官, disfavour 比劫/印; weak → favour 印/比劫, disfavour the rest; balanced → favour 食傷/財, nothing unfavourable. This is 扶抑用神 — real and common, but only one of several doctrines (調候, 病藥, 通關, 專旺/從格). **No 從格 (follow) handling**: an extremely weak DM that classically "follows" is mis-advised. The balanced tie-break is ad-hoc.
