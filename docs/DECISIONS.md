...
| **Day rollover** | `conventions.ts`; `sexagenary.ts` normalizeMoment | civil midnight **vs** 早子時 23:00 **vs** 晚子時 split | **civil_midnight** (presets `zi_23` and `split_zi` available) | **Major divergence:** a birth 23:00–23:59 gets a **different day pillar** — and thus a different hour stem via 五鼠遁. It does **not** cascade to the month or year pillar: those are derived from solar longitude and the 立春 instant at `n.utcMillis`, which the day-rollover policy never touches (an earlier version of this table claimed a possible cascade; the code does not do that). **As of Phase 10 all three positions are representable.** 早子時 (`zi_23`) rolls the whole day at 23:00; 晚子時 (`split_zi`) keeps the day pillar on the civil date but seeds the hour stem from the NEXT day via 五鼠遁 — the middle position, which is what this engine's own third-party comparator (lunar-javascript, sect 2) implements. Before it existed the engine was structurally unable to reproduce the comparator at the Zi seam, and that disagreement could only be downgraded to a non-blocking warning; `tests/verification/lunarJavascript.crosscheck.test.ts` now asserts FULL agreement under `split_zi`. The seam is also disclosed in the UI with every differing candidate chart shown side by side. Some practitioners consider the civil-midnight default *wrong* — hence showing all of them rather than asserting one. |
